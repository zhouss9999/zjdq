...
public class FtpUtil {
     ...
     * @return fileMap
     */
    public static Map<String, List<String>> getlsFileList(FTPClient ftpClient, int hour, String dbMaxTime) {
        ...
        for (int i = 0; ftpFiles != null && i < ftpFiles.length; i++) {
            FTPFile file = ftpFiles[i];
            if (file.isFile()&&file.getSize()>0) {//不是空文件
            	if(file.getName().startsWith("58646")){ //丽水站点编号
            	if (file.getName().split("_")[1].compareTo(time) >= 0) {
            		if (file.getName().contains("_JC")) {
            			jcList.add(file.getName());
            		} else if (file.getName().contains("_FY")) {
     ...
     * @return fileMap
     */
    public static Map<String, List<String>> getlxFileList(FTPClient ftpClient, int hour, String dbMaxTime) {
        ...
        for (int i = 0; ftpFiles != null && i < ftpFiles.length; i++) {
            FTPFile file = ftpFiles[i];
            if (file.isFile()&&file.getSize()>0) {//不是空文件
            	if(file.getName().startsWith("58548")){ //兰溪站点编号
            	if (file.getName().split("_")[1].compareTo(time) >= 0) {
            		if (file.getName().contains("_JC")) {
            			jcList.add(file.getName());
            		} else if (file.getName().contains("_FY")) {
     ...
     * @return fileMap
     */
    public static Map<String, List<String>> getcnFileList(FTPClient ftpClient, int hour, String dbMaxTime) {
        ...
        for (int i = 0; ftpFiles != null && i < ftpFiles.length; i++) {
            FTPFile file = ftpFiles[i];
            if (file.isFile()&&file.getSize()>0) {//不是空文件
            	if(file.getName().startsWith("58755")){ //苍南站点编号
            	if (file.getName().split("_")[1].compareTo(time) >= 0) {
            		if (file.getName().contains("_JC")) {
            			jcList.add(file.getName());
            		} else if (file.getName().contains("_FY")) {
     ...
     * @return fileMap
     */
    public static Map<String, List<String>> getkhFileList(FTPClient ftpClient, int hour, String dbMaxTime) {
        ...
        for (int i = 0; ftpFiles != null && i < ftpFiles.length; i++) {
            FTPFile file = ftpFiles[i];
            if (file.isFile()&&file.getSize()>0) {//不是空文件
            	if(file.getName().startsWith("58537")){ //开化站点编号
            	if (file.getName().split("_")[1].compareTo(time) >= 0) {
            		if (file.getName().contains("_JC")) {
            			jcList.add(file.getName());
            		} else if (file.getName().contains("_FY")) {
