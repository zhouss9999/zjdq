...
public class WeatherAnalyzeController {
	 ...
	 * @param weathertype 参数：WRF模式：[WindMap(风场)、RainMap(降雨)、PressureMap(气压)、TemperatureMap(温度)、RHMap(相对湿度)、wind-vec-black(风向箭头图-黑色)、wind-vec-white(风向箭头图-白色)],CALPUF格式：[NOX、PM2.5、PM10、SO2、mix(混合层高度)、pgt(大气稳定度)、temp(温度)、wind(风场)、wind-vec-black(风向箭头图-黑色)、wind-vec-white(风向箭头图-白色)]
	 ...
	 * @return Map
	 */
	@RequestMapping(value = "/getWRFOrCALPUF24HoursGriddingDataInfo.do", method = RequestMethod.GET, produces = { "application/json;charset=utf-8" })
	@ResponseBody
	public Map<String, Object> getWRFOrCALPUF24HoursGriddingDataInfo(HttpServletRequest request){
		String weatherType = (request.getParameter("weatherType") == null || "".equals(request.getParameter("weatherType"))) ? "" : request.getParameter("weatherType");
		String lonlat = (request.getParameter("lonlat") == null || "".equals(request.getParameter("lonlat"))) ? "" : request.getParameter("lonlat");
		String forecastDate = (request.getParameter("forecastDate") == null || "".equals(request.getParameter("forecastDate"))) ? "" : request.getParameter("forecastDate");		
		double height = (request.getParameter("height") == null || "".equals(request.getParameter("height"))) ? 0 : Double.parseDouble(request.getParameter("height"));
		String patternType = (request.getParameter("patternType") == null || "".equals(request.getParameter("patternType"))) ? "" : request.getParameter("patternType");		
		Logger.getLogger("").info("根据时间为："+forecastDate+" 获取"+patternType+"模式下气象类型为"+weatherType+"的24小时网格数据信息");
		return weatherAnalyzeService.getWRFOrCALPUF24HoursGriddingDataInfo(weatherType, lonlat, forecastDate, height, patternType);
	}
	 ...
	 * @return Map
	 */
	@RequestMapping(value = "/getWRFOrCALPUFGriddingDataFileDownload.do", method = RequestMethod.GET, produces = { "application/json;charset=utf-8" })
	@ResponseBody
	public String getWRFOrCALPUFGriddingDataFileDownload(HttpServletRequest request, HttpServletResponse response){
		String weatherType = (request.getParameter("weatherType") == null || "".equals(request.getParameter("weatherType"))) ? "" : request.getParameter("weatherType");
		String WDROrWSP = (request.getParameter("WDROrWSP") == null || "".equals(request.getParameter("WDROrWSP"))) ? "" : request.getParameter("WDROrWSP");
		String forecastTime = (request.getParameter("forecastTime") == null || "".equals(request.getParameter("forecastTime"))) ? "" : request.getParameter("forecastTime");		
		double height = (request.getParameter("height") == null || "".equals(request.getParameter("height"))) ? 0 : Double.parseDouble(request.getParameter("height"));
		String patternType = (request.getParameter("patternType") == null || "".equals(request.getParameter("patternType"))) ? "" : request.getParameter("patternType");		
		Logger.getLogger("").info("根据时间为："+forecastTime+" 获取"+patternType+"模式下气象类型为"+weatherType+"的数据文件下载");
		try {
			weatherAnalyzeService.getWRFOrCALPUFGriddingDataFileDownload(weatherType, forecastTime, height, patternType, WDROrWSP,response);
			return "文件下载成功！";
		} catch (Exception e) {
			e.printStackTrace();
			return "文件下载异常！";
